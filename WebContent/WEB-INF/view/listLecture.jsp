...
<form action = "lecture.do">  <!-- method = "GET" lecture.do?subject=??? -->
	<select name = "subject">
...
<table border = "1" style = "border-collapse: collapse;">
	<tr>
		<td colspan="4" style="text-align:center;"><a href = "lecture_write.do?subject=${num }">[게시글쓰기]</a></td>
	</tr>
	<tr>
		<th>번호</th>
		<th>제목</th>
	...
	<c:forEach var = "article" items = "${articlePage.content }">
		<tr>
			...
			<td>
			<a href = "lecture_read.do?no=${article.number }&pageNo=${articlePage.currentPage}&subject=${num}">
	...
	<c:if test = "${articlePage.hasArticles() }">
		<tr id = "nav">
			<td colspan = "4">
				...
				<a href = "lecture.do?pageNo=${articlePage.startPage -5 }&subject=${num}">[이전]</a>
				</c:if>
				<c:forEach var = "pNo" begin = "${articlePage.startPage }" end = "${articlePage.endPage }">
				<a href = "lecture.do?pageNo=${pNo }&subject=${num}">[${pNo}]</a>
				</c:forEach>
				<c:if test = "${articlePage.endPage < articlePage.totalPages }">
				<a href = "lecture.do?pageNo=${articlePage.startPage + 5 }&subject=${num}">[다음]</a>
				</c:if>
			</td>
		</tr>
	</c:if>
